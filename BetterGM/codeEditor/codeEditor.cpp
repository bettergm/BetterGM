...
void CodeEditor::setName(QString name) {
    mName = name;
}
...
void CodeEditor::lineNumberAreaPaintEvent(QPaintEvent *event) {
    ...
    while (block.isValid()) {
        ...
        //painter.drawPixmap(xofs, qRound(position.y()), mDownArrowIcon);
        //painter.drawPixmap(xofs, qRound(position.y()), mRightArrowIcon);
        ...
        if (breakPoints.contains(block.blockNumber())) {
            QLinearGradient linearGradient = QLinearGradient(xofs, qRound(position.y()), xofs + foldArea, qRound(position.y()) + foldArea);
            ...
            painter.drawEllipse(xofs + 1, qRound(position.y()), foldArea - 2, foldArea - 1);
        }
        ...
        if (/*mBookmarks->contains(block)*/bookmarks.contains(block.blockNumber())) {
            ...
            painter.setPen(Qt::NoPen);
            painter.setBrush(QBrush(linearGradient));
            painter.drawRoundRect(xofs + 1, qRound(position.y()), foldArea - 2, foldArea - 1);
        }
...
void CodeEditor::paintEvent(QPaintEvent *e) {
    ...
    for (int i = 0; i < mCursors.count(); i++) {
        QString name = mCursors.keys().at(i);
        QTextCursor cursor = mCursors.value(name);
        
        int x = cursorRect(cursor).x();
        int y = cursorRect(cursor).y();
        
        painter.setPen(Qt::white);
        painter.drawLine(x, y, x, y + 14);
        
        y -= 4;
        
        QFontMetrics met(this->font());
        int width = met.boundingRect(viewport()->rect(), Qt::AlignLeft | Qt::AlignVCenter, name).width();
        int height = met.boundingRect(viewport()->rect(), Qt::AlignLeft | Qt::AlignVCenter, name).height();
        QRect rekt(x, y - (height + 4), width + 16, height + 4);
        painter.setOpacity(0.75);
        painter.fillRect(rekt, Qt::black);
        painter.setPen(QPen(Qt::white));
        painter.drawRect(rekt);
        painter.setOpacity(1.0);
        rekt = QRect(x + 8, y  - (height + 2), width, height);
        painter.drawText(rekt, Qt::AlignLeft | Qt::AlignVCenter, name);
    }
}
...
void CodeEditor::addCursor(QString user, QTextCursor cursor) {
    mCursors.insert(user, cursor);
}

int CodeEditor::cursorCount() {
    return mCursors.count();
}
